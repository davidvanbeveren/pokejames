// Minimal PNG encoder (RGBA, no deps) for preview tooling.
...
class Canvas {
  ...
  fill(r, g, b, a = 255) { for (let i = 0; i < this.w * this.h; i++) { this.data[i * 4] = r; this.data[i * 4 + 1] = g; this.data[i * 4 + 2] = b; this.data[i * 4 + 3] = a; } }
  set(x, y, r, g, b, a = 255) { if (x < 0 || y < 0 || x >= this.w || y >= this.h) return; const i = (y * this.w + x) * 4; this.data[i] = r; this.data[i + 1] = g; this.data[i + 2] = b; this.data[i + 3] = a; }
  rect(x, y, w, h, r, g, b, a = 255) { for (let yy = y; yy < y + h; yy++) for (let xx = x; xx < x + w; xx++) this.set(xx, yy, r, g, b, a); }
  toPNG() { return encodePNG(this.w, this.h, this.data); }
}
module.exports = { encodePNG, Canvas, crc32 };
